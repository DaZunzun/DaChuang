...
class ProbCMNet(nn.Module):    #概率Unet（Unet+条件变分自编码器）+ DA网络（cm_layer 得到混淆矩阵confusion matrices）
    ...
    def elbo1(self, pred, cms, target, alpha=1, beta=0.01):  # 损失 : beta * L_KL + L_CE + alpha * L_TR （alpha=1时为热身阶段损失）
    ...
    def elbo(self, pred, cms, target, alpha=2, beta=0.01):  # 正式训练阶段总的损失 : beta * L_KL + L_CE + alpha * L_TR + L_NR
