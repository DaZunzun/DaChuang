...
class UNet_CMs(nn.Module):     #自适应矩阵以输入图像为条件的消融实验（对照组）
    """ Proposed method containing a segmentation network and a confusion matrix network.  分割网络U-net  混淆矩阵网络定义在cm_layers
    ...
    def __init__(self, in_ch, width, depth, class_no, norm='in', low_rank=False):
        ...
        self.decoders_noisy_layers = nn.ModuleList()   #解码噪声层
        #
        ...
        self.conv_last = nn.Conv2d(width, self.final_in, 1, bias=True)    #1*1卷积
        #
    ...
    def forward(self, x):
        ...
        y_noisy = []
        #
        for i in range(len(self.encoders)):   #Unet编码
            #
            y = self.encoders[i](y)
            encoder_features.append(y)
        # print(y.shape)
        for i in range(len(encoder_features)):   #Unet解码
            #
            y = self.upsample(y)
            ...
            #
            y = torch.cat([y_e, y], dim=1)   #特征拼接
            #
            y = self.decoders[-(i+1)](y)
        #
        for i in range(self.noisy_labels_no):   #直接将图像输入到DAnet，得到自适应矩阵
            #
...
class UNet_GlobalCMs(nn.Module):         #测试时固定自适应矩阵的消融实验（对照组）
...
class cm_layers(nn.Module):   # DAnet
    """ This class defines the annotator network, which models the confusion matrix.该类定义注释器网络，它对混淆矩阵建模。
    Essentially, it share the semantic features with the segmentation network, but the output of annotator network
    has the size (b, c**2, h, w)   本质上，它与分割网络共享语义特征，但注释器网络的输出大小为(b, c**2, h, w)。
    """
...
class gcm_layers(nn.Module):  #第二个消融实验使用的DAnet
...
class low_rank_cm_layers(nn.Module):    #低秩版的 DAnet
...
def double_conv(in_channels, out_channels, step, norm):    #DAnet(cm_layer)的子模块
    # ===========================================
    # in_channels: dimension of input
    # out_channels: dimension of output
    # step: stride
    # ===========================================
    if norm == 'in':       #norm决定归一化使用哪个函数进行归一化
        return nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, stride=step, padding=1, groups=1, bias=False),
            nn.InstanceNorm2d(out_channels, affine=True),  #在卷积层后添加InstanceNorm2d进行数据（一个channel内做）归一化，使得数据在进行Relu之前不会因数据过大而使网络性能不稳定
            nn.PReLU(),
        ...
        return nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, stride=step, padding=1, groups=1, bias=False),
            nn.BatchNorm2d(out_channels, affine=True),  #batch方向归一化
            nn.PReLU(),
            nn.Conv2d(out_channels, out_channels, 3, stride=1, padding=1, groups=1, bias=False),
        ...
        return nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, stride=step, padding=1, groups=1, bias=False),
            nn.GroupNorm(out_channels, out_channels, affine=True),  #将channel方向分group，然后每个group内做归一化（out_channels组）
            nn.PReLU(),
            nn.Conv2d(out_channels, out_channels, 3, stride=1, padding=1, groups=1, bias=False),
        ...
        return nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, stride=step, padding=1, groups=1, bias=False),
            nn.GroupNorm(out_channels // 8, out_channels, affine=True),    #将channel方向分group，然后每个group内做归一化（out_channels//8组）
            nn.PReLU(),
            nn.Conv2d(out_channels, out_channels, 3, stride=1, padding=1, groups=1, bias=False),
