...
data_path = "D:\\micca2018Data\\Training Set\\"     # 储存原图的文件路径
...
save_path_img = "D:\\DataSet_MICCA\\c1\\img\\"     # 储存resize之后的img.npy
save_path_lab = "D:\\DataSet_MICCA\\c1\\lab\\"     # 储存resize之后的lab.npy
save_path_nrrd = "D:\\DataSet_MICCA\\c1\\NRRD\\"   # 储存resize之后的nrrd格式的img.nrrd和lab.nrrd

save_path_imgs = "D:\\DataSet_MICCA\\c1\\imgs.npy"   # 储存拼接完成后的imgs.npy  (即某一个中心的数据)
save_path_labs = "D:\\DataSet_MICCA\\c1\\labs.npy"   # 储存拼接完成后的labs.npy
...
for i in os.listdir(data_path):
    ...
    print("处理第", num-Range_down+1 , "个样本")
    ...
    '''重采样部分'''
    # 定义原始像素尺寸和目标像素尺寸
    old_spacing = np.array(o1['spacings'])
    new_spacing = np.array([0.6, 0.6, 1.25])
    # 计算缩放因子
    zoom_factor = old_spacing / new_spacing

    # 计算重采样后的图像大小
    new_shape = np.round(image.shape * zoom_factor)

    # 使用zoom函数进行重采样
    resampled_image = zoom(image, zoom_factor, order=1)

    # 更新nrrd头文件中的像素尺寸信息
    o1['spacings'] = new_spacing.tolist()
...
# 输出拼接完成之后的尺寸
print("imgs的尺寸", imgs.shape)
print("labs的尺寸", labs.shape)
